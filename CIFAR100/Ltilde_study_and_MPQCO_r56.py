...
for n_batch in (1,2,4,8,16,32,64,98):
    ...
    for sid in range(20):
        ...
        for bitops_bound in np.linspace(21,42,43):
            # CLADO Way
            print(f'clado with bitops bound {bitops_bound}')
            ...
            v = MIQCP_optimize(cached_grad=cached_grad,
                               ...
                               bitops_bound=bitops_bound,size_bound=np.inf,
                               naive=False,PSD=True)
            ...
            # naive Way
            print(f'naive with bitops bound {bitops_bound}')
            cached_grad = Ltilde2CachedGrad(clado_ltilde)
            v = MIQCP_optimize(cached_grad=cached_grad,
                               layer_bitops=layer_bitops,
                               layer_size=layer_size,
            ...
            # MPQCO Way
            print(f'MPQCO with bitops bound {bitops_bound}')
        ...
        with open(f'RES_resnet56/RES_BITOPSOPT_resnet56{n_batch}batches(size512)_sid{sid}.pkl','wb') as f:
